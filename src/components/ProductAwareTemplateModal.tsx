...
import { X, Search, Loader2, RefreshCw, Plus, Check, ChevronDown } from 'lucide-react';
...
interface ProductAwareTemplateModalProps {
  isOpen: boolean;
  onClose: () => void;
  onInsert: (content: string, productName: string) => void;
}

const ProductAwareTemplateModal: React.FC<ProductAwareTemplateModalProps> = ({
  isOpen,
  onClose,
  onInsert
}) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const mockTemplates: GeneratedTemplate[] = selectedProducts.map(product => ({
          ...
          header: `👋 Hey {{1}}!`,
          body: `Your ${product.name} is waiting for you! 🌟\n\n${product.description}\n\nComplete your purchase now and get glowing skin! ✨\n\nLimited time offer - don't miss out! 💫`,
          footer: 'Made Safe Certified. No harmful chemicals.',
          variables: {
            '{{1}}': 'Customer Name',
            '{{2}}': 'Product Name'
          }
    ...
    try {
      ...
      setTimeout(() => {
        const newTemplate: GeneratedTemplate = {
          product: productName,
          header: `🌟 ${productName} is calling you!`,
          body: `Don't let your skincare routine wait! ${product.description}\n\nYour skin deserves the best care. Complete your order now! ✨\n\nSpecial offer just for you! 🎁`,
          footer: 'Dermatologist tested. Safe for all skin types.',
          variables: {
            '{{1}}': 'Customer Name',
            '{{2}}': 'Product Name'
          }
  ...
  const handleInsert = (template: GeneratedTemplate) => {
    const fullContent = `${template.header}\n\n${template.body}\n\n${template.footer}`;
    onInsert(fullContent, template.product);
    onClose();
  };
                    ...
                    return (
                      <div key={index} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                        ...
                        <div className="p-4 border-b border-gray-100 flex items-center gap-3">
                          ...
                          />
                          <div>
                            <h3 className="font-medium text-gray-900">{template.product}</h3>
...
export default ProductAwareTemplateModal;